...
class Products extends React.Component {
  ...
  fetchproducts = () => {
    Axios.get(
      `${API_URL}/products/?page=${this.state.page - 1}&product_name=${
    ...
    return this.state.categoryList.map((val) => {
      ...
      if (this.state.searchCategory === "") {
        return (
          <li>
            <button
              onClick={() => this.categoryHandler(val.category)}
        ...
        if (val.category === this.state.searchCategory) {
          return (
            <li>
              <button
                onClick={() => this.categoryHandler(val.category)}
                className="button-second selected"
              >
                <p>{capital}</p>
              </button>
            </li>
    ...
    return this.state.colorList.map((val) => {
      ...
      if (this.state.searchColor === "") {
        return (
          <li>
            <button
              onClick={() => this.colorHandler(val.color)}
              className="button-second"
            >
              <p>{capital}</p>
            </button>
        ...
        if (val.color === this.state.searchColor) {
          ...
          return (
            <li>
              <button
                onClick={() => this.colorHandler(val.color)}
                className="button-second unselected"
              >
                <p>{capital}</p>
              </button>
            </li>
  ...
  fetchMaxPage = () => {
    Axios.get(
      `${API_URL}/products/max-page?category=${this.state.searchCategory}&color=${this.state.searchColor}&product_name=${this.props.userGlobal.searchProduct}`
    )
      .then((result) => {
        this.setState({
          maxPage: Math.ceil(
  ...
  fetchFilteredProducts = () => {
    ...
    Axios.get(
      `${API_URL}/products/?page=${this.state.page - 1}&sortby=${
        this.state.sortProduct
      }&category=${this.state.searchCategory}&color=${
        this.state.searchColor
      }&product_name=${this.props.userGlobal.searchProduct}`
    )
      .then((result) => {
        ...
        <div className="row">
          <div className="col-2 filter-bar">
            <div>
              ...
              <h3>SHOP BY COLOR</h3>
              <ul>{this.renderColor()}</ul>
            </div>
            <div>
              <button
                className="btn btn-dark btn-sm filter"
                onClick={this.fetchFilteredProducts}
              >
                <p>Filter</p>
              </button>
              <button
          ...
          <div className="col-10 ">
            ...
            {this.state.productList.length === 0 ? (
              <div className="d-flex align-items-center flex-row justify-content-center mt-5">
                <h4>
                  Sorry, none of our collections match your search. Please try
                  again.
                </h4>
              </div>
            ) : (
